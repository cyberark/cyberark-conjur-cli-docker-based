...
describe Conjur::Command::Policy do
  context logged_in: true do
    ...
    describe_command 'policy:load --collection the-collection --id the-policy -v 1.0.0 policy-body' do
    ...
    context "default collection" do
      ...
      describe_command 'policy:load --id the-policy -v 1.0.0 policy-body' do
        it "creates the policy with default collection" do
          invoke.should == 0
        end
      end
      describe_command 'policy:load --name "The Policy" --id the-policy -v 1.0.0 policy-body' do
        let(:name) { "The Policy" }
        it "creates the policy with default collection" do
          invoke.should == 0
        end
